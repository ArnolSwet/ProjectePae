...
struct RxReturn RxPacket(void) {
    ...
    //Comprovem que el Checksum calculat sigui el mateix que el que hem guardat en el StatusPacket calculat en la funció de transmissió
    if (bCheckSum != respuesta.StatusPacket[bPacketLength - 1]) {

        // Si el CheckSum no és el mateix, vol dir que tenim un error.
